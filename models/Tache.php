...
class Tache {
     ...
     * Initialise la connexion à la base et charge la tâche si un ID est fourni
     */
    ...
    /**
     * Charge une tâche depuis la base de données à partir de son identifiant
    ...
    /**
     * Modifie une tâche existante avec les nouvelles données
    ...
    /**
     * Supprime la tâche courante de la base de données ainsi que ses notifications associées
    ...
    /**
     * Change le statut de la tâche (à faire, en cours, terminée)
    ...
    /**
     * Définit la priorité de la tâche (basse, moyenne, haute)
    ...
    /**
     * Définit le temps estimé pour la tâche
     * @param int $temps Temps estimé en minutes
     * @return bool Succès du changement
     */
    public function definirTempsEstime($temps) { // enregitre ou modifie le temps estimé pour une tâche 
        if (!is_numeric($temps) || $temps < 0) { // Vérifie si le temps est un nombre et est positif
            return false;
        }
        return $this->modifier(['temps_estime' => $temps]); // Modifie le temps estimé de la tâche
    }
    
    /**
     * Récupère toutes les tâches d'un utilisateur avec des filtres éventuels
    ...
    public static function getTachesUtilisateur($id_utilisateur, $filtres = []) {
        ...
        $taches = []; //stocke les taches trouvées (Pour chaque tâche trouvée,la fonction crée un objet Tache contenant toutes ses informations)
    ...
    /**
     * Crée une notification liée à la tâche (si date d'échéance renseignée)
     * @return bool Succès de la création
     */
    ...
    /**
     * Met à jour la notification associée à la tâche (en cas de changement de date d'échéance)
